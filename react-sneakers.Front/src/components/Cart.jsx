    ...
    <div className="cart__total">
      ...
      <div className="cart__total__tax d-flex">
        ...
        <span className="cart__total__tax-number">
          $ {(totalPrice * 0.08).toFixed(1)}
        </span>
      </div>
      <button
        onClick={() => makeOrder(cartItems)}
        type="button"
        className="green-button cart__total__submit"
      >
        Buy now
          ...
          <div className="cart__body" ref={cartBodyRef} onClick={onCloseCart}>
            <div className={`cart__content ${classTransitions}`}>
              <h3 className="cart__title">
                Cart
                <img
                  src="/react-sneakers/img/close-cart.png"
                  alt="close cart"
              ...
              {cartItems.length > 0 ? (
                ...
                <CartInfo
                  ...
                  image="/react-sneakers/img/order-completed.jpg"
                  text={`Your order #${orderNumber} will soon be transferred to the delivery service`}
                  completed
                />
              ) : (
                <CartInfo
                  title="Cart is empty"
                  image="/react-sneakers/img/cart-empty.png"
                  text="Add at least one item to the order"
                />
              )}
